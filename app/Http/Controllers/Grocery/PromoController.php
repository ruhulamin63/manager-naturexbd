            ...
            // if ($this->hasPermission($request, 'add_city')) {
                ...
                if ($validator->fails()) {
                    ...
                    foreach ($cityList as $city) {
                        $exisitng = DB::table('grocery_promo')
                        ...
                        if (count($exisitng) == 0) {
                            ...
                            $insert = DB::table('grocery_promo')->insert($data);
                        }
                    }
                    return redirect()->back()->with([
                        'error' => false,
        ...
        if ($this->isLoggedIn($request)) {
            // if ($this->hasPermission($request, 'update_city')) {
                ...
                if ($validator->fails()) {
                    ...
                    $update = DB::update("UPDATE grocery_promo SET status= '$promoStatus' WHERE id = '$promo_id';");
        ...
        if ($this->isLoggedIn($request)) {
            // if ($this->hasPermission($request, 'update_city')) {
                ...
                if ($validator->fails()) {
                    ...
                    $promo_id = $request->input('promo_id');
                    $promoStatus = $request->input('promo_status');
                    $update = DB::update("UPDATE grocery_promo SET status= '$promoStatus',activeSatus= '$promoStatus' WHERE id = '$promo_id';");
                    //$update = City::where('id', $cityID)->update(['status' => $request->input('city_status')]);
                    if ($update) {
        ...
        if ($this->isLoggedIn($request)) {
            // if ($this->hasPermission($request, 'add_city')) {
                ...
                if ($validator->fails()) {
                    ...
                    $data=DB::update("UPDATE grocery_promo SET promo_code= '$promoCode',count= '$promo_count',start= '$start_date',end= '$end_date',conditions= '$promo_con',amount='$Amount',promo_type='$promo_type',conditions_amount='$MinAmount' WHERE id = $promoId;");
                    if($data==null){
                        return redirect()->back()->with([
                            'error' => true,
                            'message' => 'Something went wrong.'
        ...
        if ($this->isLoggedIn($request)) {
            // if ($this->hasPermission($request, 'add_city')) {
                ...
                if ($validator->fails()) {
                    ...
                    $promo_info=DB::table('grocery_promo')
                        ->where('id',$promoId)
                        ->first();
                    ...
                    $imageData='/promo_banner/'.$image_full_name;
                        ...
                        $data=DB::update("UPDATE grocery_promo SET image= '$imageData' WHERE id = $promoId;");
                        if($data==null){
                            return redirect()->back()->with([
                                'error' => true,
